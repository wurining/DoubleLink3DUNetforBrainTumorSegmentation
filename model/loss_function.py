...
class MainLossFunction(tf.keras.losses.Loss):
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        # print('Calculate main_loss_function')
        # print('Calculate main_loss_function y_true', y_true.shape)
        # print('Calculate main_loss_function y_pred', y_pred.shape)
        ...
        # lambda_1 = 1 * (wt1 * NCR_loss + wt2 * ED_loss + wt3 * ET_loss) / (wt1 + wt2 + wt3)
        lambda_2 = 1 * tf.keras.losses.CategoricalCrossentropy()(y_true, y_pred)
        # lambda_3 = 0.5 * ModalityPairingLoss(data_type=self.dtype_mix)([0], [a, b])
        # tf.print('main_loss_function_1 DiceLoss', lambda_1)
        # tf.print('Norm_loss DiceLoss', Norm_loss)
        # tf.print('NCR_loss DiceLoss', NCR_loss)
        # tf.print('ED_loss DiceLoss', ED_loss)
        # tf.print('ET_loss DiceLoss', ET_loss)
        # tf.print('main_loss_function_2 CategoricalCrossentropy', lambda_2)
...
class AuxLossFunction(tf.keras.losses.Loss):
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        ...
        # lambda_1 = 1 * (wt1 * NCR_loss + wt2 * ED_loss + wt3 * ET_loss) / (wt1 + wt2 + wt3)
        lambda_2 = 1 * tf.keras.losses.CategoricalCrossentropy()(y_true, y_pred)
        # tf.print('aux_loss_function_1 DiceLoss', lambda_1)
        # tf.print('aux_loss_function_2 CategoricalCrossentropy', lambda_2)
        return lambda_1 + lambda_2
...
class DiceLoss(tf.keras.losses.Loss):
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        # tf.print('y_true dice', y_true.shape)
        # tf.print('y_pred dice', y_pred.shape)
        # shape 2B whd 4C
        ...
        scale = 1
        if scale != 1:
            # tf.print('Dice y_true.shape', y_true.shape)
            if not batch:
                return result
            y_true = tf.multiply(y_true, scale)
            y_pred = tf.multiply(y_pred, scale)
            for i in range(batch):
                # y_pred[i] = tf.math.sigmoid(y_pred[i])
                numerator = (2 * tf.reduce_sum(y_true[i] * y_pred[i])) / scale + smooth * scale
                denominator = tf.reduce_sum(y_true[i] + y_pred[i])
                # tf.print('Dice numerator', numerator)
                # tf.print('Dice denominator', denominator)
                # tf.print('Dice *****', y_true[i] * y_pred[i])
...
class ModalityPairingLoss(tf.keras.losses.Loss):
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        ...
        # tf.print('ModalityPairingLoss loss', result)
        return result / batch
